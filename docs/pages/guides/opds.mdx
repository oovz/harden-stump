...
<Callout type="warning">
	**Secure Libraries are not accessible via OPDS.** Content in [Secure Libraries](/guides/secure-libraries) requires client-side decryption, which OPDS clients cannot perform. Secure library content will not appear in OPDS catalogs.
</Callout>
